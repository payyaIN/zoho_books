...
class UpdateVendorFormNotifier extends StateNotifier<AddVendorModel> {
  UpdateVendorFormNotifier()
  ...
  void updateBillingAddress(String key, dynamic value) {
    final updatedAddress = Map<String, dynamic>.from(state.billingAddress);
    updatedAddress[key] = value;
    state = state.copyWith(billingAddress: updatedAddress);
  }

  void updateShippingAddress(String key, dynamic value) {
    final updatedAddress = Map<String, dynamic>.from(state.shippingAddress);
    updatedAddress[key] = value;
    state = state.copyWith(shippingAddress: updatedAddress);
  }
  ...
  void _validateAddress(
      Map<String, dynamic> address, String prefix, Map<String, String> errors) {
    // Normalize values
  ...
  void validateFieldsAndUpdateState() {
    ...
    final phoneRegex =
        RegExp(r'^5\d{8}$'); // Saudi mobile: 9 digits starting with 5
    final saudiZipRegex =
        RegExp(r'^[1-9]\d{4}$'); // Saudi ZIP: 5 digits, cannot start with 0
    ...
    if (state.email.trim().isNotEmpty &&
        !emailRegex.hasMatch(state.email.trim())) {
    ...
    final billingBuilding =
        (state.billingAddress['building'] ?? '').toString().trim();
    final shippingBuilding =
        (state.shippingAddress['building'] ?? '').toString().trim();

    if (billingBuilding.isNotEmpty &&
        !buildingRegex.hasMatch(billingBuilding)) {
      errors['billing_building'] = 'Building number must be exactly 5 digits.';
    }
    if (shippingBuilding.isNotEmpty &&
        !buildingRegex.hasMatch(shippingBuilding)) {
    ...
    if (billingZip.isNotEmpty && !saudiZipRegex.hasMatch(billingZip)) {
      errors['billing_zip'] =
          'Invalid Saudi ZIP code (must be 5 digits and cannot start with 0).';
    }
  ...
  void clearForm() {
    state = UpdateVendorFormNotifier().state;
  }
}

final updateVendorFormProvider =
    StateNotifierProvider<UpdateVendorFormNotifier, AddVendorModel>((ref) {
  return UpdateVendorFormNotifier();
});
