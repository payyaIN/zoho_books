...
class ExpenseImportExportRepository {
  ...
  /// Step 0: Download sample expense XLS file
  Future<DownloadSampleResponse> downloadSampleFile() async {
    try {
      final result = await _apiService.getApi(
        url:
  ...
  /// Step 1: Validate uploaded expense file and get field mapping suggestions
  Future<ValidateFileResponse> validateFile({
    required File file,
  }) async {
    try {
      final result = await _apiService.postMultipartFile(
        url:
            "http://81.208.173.149/pb-accounting-service/api/bulkExpenseImport/validateFile",
        file: file,
        fileFieldName: 'file',
  ...
  /// Step 2: Preview the data before importing
  Future<Map<String, dynamic>> previewImport({
    ...
    try {
      final result = await _apiService.postMultipartWithBlobJson(
        url:
            "http://81.208.173.149/pb-accounting-service/api/bulkExpenseImport/previewImport",
        file: file,
        additionalFields: {
          'config': config,
          'mapping': mapping,
  ...
  /// Step 3: Import uploaded expense file (Final Import)
  Future<ImportFileResponse> importFile({
    required File file,
    required Map<String, dynamic> config,
    required Map<String, dynamic> mapping,
