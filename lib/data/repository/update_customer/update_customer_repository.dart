...
// File: lib/data/repository/update_customer/update_customer_repository.dart
// ✅ FIXED VERSION - Includes addressId for billing and shipping addresses
...
class UpdateCustomerRepository {
  ...
  Future<BaseApiResponse> updateCustomer({required int partyId}) async {
    ...
    // primary contact arabic
    final String primaryFirstArabic =
        state.primaryContactArabic?.firstNameArabic ?? '';
    final String primaryLastArabic =
        state.primaryContactArabic?.lastNameArabic ?? '';
    ...
    final customerBody = <String, dynamic>{
      ...
      "customerType":
          (state.customerType ?? 'BUSINESS').toString().toUpperCase(),
      "partyType": (state.partyType ?? 'CUSTOMER'),
      "partyId": partyId.toString(),
      "companyName": state.companyName ?? '',
      "companyNameArabic": state.companyNameArabic ?? '',
      "displayName":
          (state.displayName != null && state.displayName!.isNotEmpty)
              ? state.displayName
              : (state.companyName ?? ''),
      "emailAddress": state.emailAddress ?? '',
      "phoneCode": state.phoneCode ?? '',
      ...
      // ✅ CRITICAL FIX: Include addressId for both billing and shipping addresses
      "billingAddress": {
        "addressId": billing?.addressId, // ✅ ADDED: Required by API for updates
        "countryRegion": billing?.countryRegion ?? '',
        "buildingNumber": billing?.buildingNumber ?? '',
        "streetName":
            billing?.streetName, // ✅ CHANGED: Use null instead of empty string
        "streetAddress": billing?.streetAddress ?? '',
        "streetAddressArabic": billing?.streetAddressArabic ?? '',
        "city": billing?.city ?? '',
        "cityArabic": billing?.cityArabic ?? '',
        "state": billing?.state, // ✅ CHANGED: Use null instead of empty string
        "zipCode": billing?.zipCode ?? '',
      },
      "shippingAddress": {
        "addressId":
            shipping?.addressId, // ✅ ADDED: Required by API for updates
        "countryRegion": shipping?.countryRegion ?? '',
        "buildingNumber": shipping?.buildingNumber ?? '',
        "streetName":
            shipping?.streetName, // ✅ CHANGED: Use null instead of empty string
        "streetAddress": shipping?.streetAddress ?? '',
        "streetAddressArabic": shipping?.streetAddressArabic ?? '',
        "city": shipping?.city ?? '',
        "cityArabic": shipping?.cityArabic ?? '',
        "state": shipping?.state, // ✅ CHANGED: Use null instead of empty string
        "zipCode": shipping?.zipCode ?? '',
      },
      "contactPersons": contactPersonsPayload,
      "sameAddressFlag": state.sameAddressFlag ?? false,
    ...
    debugPrint("📤 Update Customer Request Body:");
    debugPrint("🔑 Billing AddressId: ${billing?.addressId}");
    debugPrint("🔑 Shipping AddressId: ${shipping?.addressId}");
